...
description: EDUQG 出题 Skill。用于把教师出题需求归一化为 edu-question-spec.v1，并生成可校验的结构化课堂题目；支持文本题、答案相关图文/SVG 题、EvoQ GA 候选优化、质量评估、离线 mock、本地 HTTP 服务和 OAH 兼容工具调用。
---
...
3. 生成题目。
   ...
   - 每种算法对应一个算法智能体/策略：`direct-question-agent`、`cot-question-agent`、`react-question-agent`、`dear-question-agent`、`eqpr-question-agent`、`evoq-question-agent`。
...
对外按 `1 个主智能体 + 6 个算法智能体 + 服务能力` 理解：

- 主智能体：负责对话抽字段、更新画像、spec ready gating 和路由。
- 六算法智能体：`direct`、`cot`、`react`、`dear`、`eqpr`、`evoq` 都是核心能力，不能合并掉业务行为。
- 服务能力：生成、评估、EvoQ 学生模拟、画像持久化可以在内部用兼容 subagents/tools 承担，不需要作为产品层 8 个独立智能体暴露。
- OAH 里的 8 个 `subagents` 和 9 个原工具名是兼容合同；收口阶段只合并暴露面和文档口径，不删旧名字、不改业务逻辑。

## 工具边界

基础出题不需要理解所有旧 Agent 工具。推荐主入口是：

```http
POST /api/eduqg/generate
```

OAH 兼容层保留 9 个原工具名，是为了不破坏 Tutor 前端、回归脚本和旧部署契约。六种算法是核心路径；`simulate_student_response` 是 EvoQ 的必需能力；`read_profile`/`write_profile` 属于画像持久化接口。Tutor 正式画像工作流使用 `portrait_id` 和 portrait store 持久化生成记录；独立 skill/OAH 工具服务使用这两个接口读写画像提示。

`settings.yaml` 中看起来有 22 个 schema refs，是因为 11 个 HTTP 工具/兼容端点分别声明了 input 和 output；唯一 schema 文件只有 2 个：
